...
import { registerUser, loginUser, getUserFromToken,getAllUsersFromService } from '../services/authService.js';
...
export const getAllUsers = async (req, res) => {
    ...
    try {
        const response = await getAllUsersFromService(token);  // Call the service

        if (response.success) {
            return res.status(200).json({ success: true, users: response.users });
        } else {
            return res.status(401).json({ success: false, message: response.message });
        }
    } catch (error) {
